...
parser.add_argument('--dataPath', type=str, default='/mnt/pank/netvlad-mult-domains/netvlad/data/', help='Path for centroid data.')
parser.add_argument('--runsPath', type=str, default='/mnt/pank/netvlad-mult-domains/netvlad/runs/', help='Path to save runs to.')
...
parser.add_argument('--encoderPath', type=str, default='/mnt/pank/netvlad-mult-domains/ToDayGAN/checkpoints/night2day_l2/',
...
def train(epoch):
    ...
    whole_training_dl = whole_training_data_loader
...
if __name__ == "__main__":
    ...
    if opt.mode.lower() == 'train':
        ...
        whole_train_set = dataset.get_whole_training_set(opt.arch.lower())
        whole_training_data_loader = DataLoader(
            dataset=whole_train_set, num_workers=opt.threads, batch_size=opt.cacheBatchSize,
        ...
        print('====> Training query set:', len(train_set_list))
            ...
            if not opt.resume:
                if opt.mode.lower() == 'train':
                    initcache = join(
                        ...
                        opt.arch.lower() + '_' + train_set_list.dataset
